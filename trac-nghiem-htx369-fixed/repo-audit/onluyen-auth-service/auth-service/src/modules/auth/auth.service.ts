...
export const authService = {
  ...
  /**
   * Authenticated "change my own password" flow (distinct from the
   * unauthenticated forgot/reset flow above). Requires the current
   * password to be re-entered — this is what stops a hijacked but
   * still-valid access token from silently locking the real owner out.
   * All refresh tokens (every device/session) are revoked afterward,
   * including the one used to make this request, so the caller must
